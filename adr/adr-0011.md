...
Antes de aceptar el ADR se realizará una **PoC de visitantes offline, reservas concurrentes y push**, con pruebas de Angular y Playwright. Deberá validar persistencia, reintentos sin duplicados, aislamiento, expiración de sesión, actualización del worker y objetivos de UX y rendimiento. La sincronización en diez segundos se comprobará con volumen definido, aplicación activa, sesión válida y API disponible; no se garantiza ese tiempo con el navegador cerrado. Las versiones compatibles se fijarán durante la PoC y la elección se revisará si el aprendizaje compromete las entregas.
